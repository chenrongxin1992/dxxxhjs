...
function PDF_launch(ob, zzw, zzh, callback, titlestr) {
    ...
    if (!titlestr) {
        PDF_c.innerHTML =
            "<a style='background:url(/images/bt_closed.gif) no-repeat;width:30px;height:30px;margin:-10px -18px 0 0;display:inline;position:relative;float:right;cursor:pointer;' onclick='PDF_close();return false;'></a>";
    } else {
        if (titlestr == "no") {
            PDF_c.innerHTML = "";
        } else {
            ...
            if (needDefine) {
                ...
                color = "#ebf5ff";
                bg =
                    "background:url(/images/icon_popup_closenew.png) no-repeat;width:17px; height:17px;margin-top:10px;";
            }
            PDF_c.innerHTML = '<div style="background:' + bkcolor +
                ';padding:0 20px;height:38px;line-height:38px;border-radius:10px 10px 0 0;" class="navbox"><span style="color:' +
                color + ';float:left;font-size:16px;">' + titlestr +
                '</span><a href="javascript:void(0);" onclick="PDF_close();return false;" style="' + bg +
                'float:right;overflow:hidden;" class="pdfclosebtn" title="关闭"></a></div>';
            titleHeight = 14;
        }
    }
